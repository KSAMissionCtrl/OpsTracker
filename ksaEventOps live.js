...
function loadEventsAJAX(xhttp) {
    var isAscentActive = false;
  
    // if there is a vessel being loaded and it's not yet loaded, wait
    if (getParameterByName("vessel").length && !ops.currentVessel) { setTimeout(loadEventsAJAX, 100, xhttp); return; }
  
    // if a vessel is loaded and has telemetry & the telemetry should be running now...
    if (ops.currentVessel && ops.currentVessel.AscentData.length && currUT() >= ops.currentVessel.AscentData[0] && ops.currentVessel.AscentData[ops.currentVessel.AscentData.length-1]-15 >= currUT()) {
  
      // make sure it is loaded
      if (!ops.ascentData.telemetry.length) { setTimeout(loadEventsAJAX, 100, xhttp); return; }
      else isAscentActive = true;
    }
    ...
    if (launches[0] != "null" && !isAscentActive) {
    ...
    // is there an active launch?
    } else if (launches[0] != "null" && isAscentActive) {
  
      // find the vehicle launching and use its data
      launches.forEach(function(launchEvent) {
        var launchDetails = launchEvent.split(";");
        if (isAscentActive && launchDetails[2] == ops.ascentData.vessel) {
          writeLaunchInfo({
            UT: parseFloat(launchDetails[0]),
            LaunchTime: parseFloat(launchDetails[1]),
            db: launchDetails[2],
            Title: launchDetails[3],
            Desc: launchDetails[4]
          }, true);
          isAscentActive = false;
        }
      });
  ...
  function writeLaunchInfo(data, isAscentActive = false) {
    ...
    if (data) {
      ...
      // active launch, regular launch, or hold event?
      if (isAscentActive) {
